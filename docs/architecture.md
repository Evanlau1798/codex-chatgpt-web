...
launcher-owned codex-chatgpt-web daemon
  ├─ official /models passthrough + grouped and legacy ChatGPT Web models
...
the temporary auth view. Embedded sign-in remains the default. When normal Chrome is available, the
launcher also offers an explicit Passkey fallback that uses a dedicated temporary Chrome profile,
imports only allowlisted non-partitioned session cookies, re-verifies the authenticated composer,
and removes the profile. It never copies a user's normal browser profile or silently falls back.
...
The setting applies to Codex and Claude task conversations. API Access stays Temporary Chat in
both its direct worker and native tool bridge; `/v1/chat/completions` still rejects `store:true`.
Changing this preference does not delete previously saved ChatGPT history.

Original Automatic mode also offers an explicit **New browser chat for each turn** setting, disabled by
...
forced off in Enhanced and Zero Risk modes. The UI disables it and launcher/config/adapter
boundaries reject or clear incompatible values, including stale configuration. API tool-result
HTTP rounds remain part of one native Web generation and never enable this preference.
...
Interactive mode changes and ordinary launcher lifecycle operations block while a browser or manual
turn owns a user-visible surface. HTTP-only turns do not block the fork's controlled atomic restart:
new admission stops, the launcher terminates the old runtime, and clients reconnect to the newly
built candidate. The launcher stops the tunnel through its runtime command and asks the daemon to
flush state and exit through an authenticated shutdown endpoint when available. If ownership or
cleanup cannot be established safely, the operation fails closed and restores the drained runtime
when possible. An unexpected child exit is recovered with a bounded restart budget; a crash loop
becomes an explicit launcher error.
