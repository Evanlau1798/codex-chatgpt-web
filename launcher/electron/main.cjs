...
const GITHUB_URL = "https://github.com/Evanlau1798/codex-chatgpt-web";
const X_URL = "https://x.com/EvanLau1798";
...
function registerIpc({ logger, stateStore }) {
  ...
  handle("launcher:snapshot", async () => {
    ...
    return {
      ...
      connectorNames: {
        automatic: runtimeHost.browserConnectorName("automatic"),
        manual: runtimeHost.browserConnectorName("manual"),
      },
  ...
  handle("launcher:mcp-verify", async (event) => {
    ...
    if (stateStore.read().browserInteractionMode === "manual") {
      ...
      send("launcher:state-changed", state);
      publishOperation({ name: operationName, status: "completed", message: "Local Zero Risk runtime is healthy" });
      return report;
    }
    try {
  ...
  const setupIntegration = async (integration) => {
    const manual = stateStore.read().browserInteractionMode === "manual";
    const browser = manual ? { authenticated: true } : await browserHost.probeAuthentication();
    if (!manual && !browser.authenticated) {
      ...
      throw new Error(
        IS_DEV_PROFILE
          ...
          : "Sign in to ChatGPT before installing an integration",
      );
    }
    const setupState = stateStore.read();
    if (!manual && !setupState.coreSetupComplete
      ...
      throw new Error(
        IS_DEV_PROFILE
          ...
          : "Run the browser smoke test before installing an integration",
      );
    }
    const result = IS_DEV_PROFILE ? await runtimeHost.setupDevCore() : await runtimeHost.setupCore(integration);
    const installsCodex = IS_DEV_PROFILE || integration === "codex";
    stateStore.update({
      bridgeEnabled: IS_DEV_PROFILE ? false : installsCodex
        ? true
...
void start().catch(async (error) => {
  ...
  try {
    ...
    const options = {
      ...
      detail: [
        copy.startupDetail,
        ...cleanupErrors.map(detail => `${copy.startupCleanupFailed}: ${detail}`),
      ].join("\n"),
