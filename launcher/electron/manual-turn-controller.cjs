...
function isFirstConversationNavigation(tab, url) {
  if (tab.manualConversationReused
    || tab.manualInitialConversationNavigationAccepted
    || !["sent", "running"].includes(tab.manualState)) return false;
  try {
    const previous = new URL(tab.url);
    const next = new URL(url);
    return previous.origin === "https://chatgpt.com" && next.origin === previous.origin;
  } catch {
    return false;
  }
}
...
class ManualTurnController {
  ...
  begin(traceId, helperPid, prompt, conversationKey, resumePrompt, compaction = false, refreshPrompt, systemRevision) {
    ...
    if (terminal?.helperPid === helperPid) {
      throw Object.assign(new Error(`Zero Risk turn ${traceId} was already ${terminal.status}`), {
        code: terminal.status === "timeout" ? "manual_turn_timed_out"
          : terminal.status === "cancelled" ? "turn_cancelled" : "manual_turn_failed",
      });
    }
  ...
  navigation(tab, url, inPlace) {
    ...
    if (isFirstConversationNavigation(tab, url)) {
