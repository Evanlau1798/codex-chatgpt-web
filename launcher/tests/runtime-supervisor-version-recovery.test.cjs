...
test("daemon recovery stops before spawn when configured release is stale", async () => {
  ...
  supervisor.readConfig = () => ({ releaseVersion: "2.1.6", mode: "browser-only" });
  ...
  assert.deepEqual(states, [{
    status: "needs-setup",
    detail: "Config requires 2.1.6; launcher is 3.0.0",
  }]);
  assert.deepEqual(operations.at(-1), {
    name: "runtime-recovery",
    status: "failed",
    message: "Config requires 2.1.6; launcher is 3.0.0",
  });
});
