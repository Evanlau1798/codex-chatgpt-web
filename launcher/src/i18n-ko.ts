export const ko = {
  ...
  savedChatsBody: "Codex와 Claude 작업 대화를 ChatGPT 기록에 보관합니다. 메모리와 맞춤 지침이 적용될 수 있습니다. API 요청은 항상 임시 채팅을 사용합니다. 기본값은 꺼짐입니다.",
  ...
  zeroRiskProProfileInfo: "컴팩션 전에 약 250,000개의 사용 가능한 토큰을 제공합니다. 매 턴마다 ChatGPT Pro를 직접 선택하세요. ChatGPT Pro 계정이 필요하며, 계정이 없으면 턴이 실패할 가능성이 높습니다. 또한 늘어난 한도가 모든 effort에서 동작하지 않을 수도 있습니다. Zero Risk는 구독 상태나 선택한 모델을 확인할 수 없습니다.",
  ...
  accountSafetyActiveWebTurn: "Web 작업이 진행 중입니다. 작업이 끝날 때까지 계정 안전 설정을 변경할 수 없습니다.",
  ...
  apiAccess: "API 액세스(실험적)",
  apiAccessSummary: "에이전트 도구용 로컬 OpenAI 호환 엔드포인트입니다. 안정성을 검증 중입니다.",
  apiAccessToggle: "API 액세스 사용",
  apiAccessHelpLabel: "API 액세스 위험 안내",
  apiAccessWarningTitle: "API 액세스를 켜기 전에 확인",
  apiAccessWarningBody: "이 기능은 실험적 엔드포인트이며 OpenAI 공식 API가 아닙니다. 호환성이 아직 완전하지 않고 동작이 불안정할 수 있습니다. 응답이 지연되거나 실패할 수 있으며, 도구 호출과 여러 턴에 걸친 에이전트 작업이 모든 클라이언트에서 안정적으로 작동하지 않을 수 있습니다. 중요한 작업에는 사용하지 마세요. 이 엔드포인트는 Pi 같은 에이전트 인터페이스와 외부 harness용입니다. Account Safety와 함께 사용하되 로컬 제한으로 ChatGPT의 속도 제한을 완전히 막을 수는 없습니다. API 키는 신뢰할 수 있는 로컬 도구에만 제공하고 공개하지 마세요. 대화마다 시간이 오래 걸릴 수 있으며 요청이 잦으면 429 Too many requests가 발생할 수 있습니다. 429 또는 계정 보안 경고가 나타나면 요청을 중단하고 계정 상태를 확인하세요. 부적절한 사용으로 인한 계정 위험은 사용자가 부담하며, 그 결과 발생한 계정 제한·정지·차단에 대해 프로젝트와 개발자는 책임지지 않습니다.",
  apiAccessWarningCancel: "취소",
  apiAccessWarningConfirm: "이해했습니다. 사용",
