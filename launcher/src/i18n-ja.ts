...
export const ja: Record<keyof Copy, string> = {
  ...
  noAutoCompactBody: "試験的機能です。ルーティングされた ChatGPT Web モデルのコンテキストウィンドウと自動コンパクションしきい値を Codex から隠し、Codex による自動コンパクションを無効にします。ChatGPT Web 自体のメッセージ、コンポーザー、モデル、サービス上限は引き続き適用されます。Token 使用量は正しく記録され、大きすぎるブラウザープロンプトは安全に失敗します。変更後は Codex を再起動してください。初期設定では無効です。",
