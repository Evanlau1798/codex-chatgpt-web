...
const en = {
  ...
  stepInstall: "Install into Codex + Claude Code",
  stepInstallBody: "Add the ChatGPT Web models to Codex and replace Claude Code's model picker with the routes available to this ChatGPT account. Previous settings are restored on removal.",
...
const zh: Record<keyof typeof en, string> = {
  ...
  runSmoke: "运行冒烟测试",
  smokePassed: "冒烟测试通过",
  stepInstall: "安装到 Codex + Claude Code",
  stepInstallBody: "将 ChatGPT Web 模型添加到 Codex，并用当前 ChatGPT 帐号可用的路由替换 Claude Code 模型列表。移除集成时会恢复原有设置。",
  install: "安装模型",
  reinstall: "重新安装模型",
