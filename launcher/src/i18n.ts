...
const en = {
  ...
  biggerContextBody: "Available only when Enhanced Web session mode is off. Small turns use one message; larger turns use two or six messages in total, with only the final message executing the task. Every message is a real ChatGPT request and may consume account allowance. It triples the context and compaction thresholds advertised to Codex; ChatGPT message, model, composer and service limits still apply. This is not unlimited context. Restart Codex after changing it.",
  ...
  noAutoCompactBody: "Removes only the ChatGPT Web context-window and automatic-compaction thresholds advertised to Codex. Enhanced mode keeps a private recovery checkpoint at safe tool-result boundaries (about every 100K tokens on Pro, earlier on Plus). The current page continues; a failed page rebuilds from the checkpoint and later canonical work. ChatGPT limits still apply, and oversized prompts fail closed. Restart Codex after changing it. Disabled by default.",
  savedChats: "Save chats in ChatGPT",
  savedChatsBody: "Keep Codex and Claude task conversations in ChatGPT history. ChatGPT memory and custom instructions may apply. API requests always use Temporary Chat. Off by default.",
  ...
  manualFreshConversationUnavailable: "Available only with Automation and Original session mode. Enhanced and Zero Risk force this setting off.",
  ...
  apiAccess: "API Access (experimental)",
  apiAccessSummary: "Local OpenAI-compatible endpoint for agentic tools. Reliability is still being tested.",
  apiAccessToggle: "Enable API Access",
  apiAccessHelpLabel: "About API Access risks",
  apiAccessWarningTitle: "Before enabling API Access",
  apiAccessWarningBody: "This experimental endpoint is not the official OpenAI API. Compatibility is incomplete and behavior may be unstable: responses can be delayed or fail, and tool calls or multi-turn agent workflows may not work reliably with every client. Do not rely on it for critical work. It is intended for external harnesses such as Pi and other agentic interfaces. Use Account Safety alongside it, but local limits cannot guarantee that ChatGPT will not rate-limit you. Share the API key only with trusted local tools; never publish it. Each conversation may take a long time, and frequent requests can trigger 429 Too many requests. If you see a 429 or an account-security warning, stop sending requests and check your account. You bear the account risks of improper use; the project and developer are not responsible for resulting account restrictions, suspension, or blocks.",
  apiAccessWarningCancel: "Cancel",
  apiAccessWarningConfirm: "I understand, enable",
  ...
  accountSafetyActiveWebTurn: "A Web turn is running. Account Safety settings are locked until it finishes.",
...
const zh: Record<keyof typeof en, string> = {
  ...
  biggerContextBody: "仅可在增强型 Web 会话模式关闭时使用。小回合发送一条消息，大回合共发送两条或六条消息，只有最后一条执行任务。每条都是真实的 ChatGPT 请求，可能消耗账户额度。向 Codex 公布的上下文和压缩阈值扩大三倍，但 ChatGPT 的消息、模型、编辑器和服务限制仍有效。这不是无限上下文。更改后请重启 Codex。",
  ...
  noAutoCompactBody: "仅移除向 Codex 公布的 ChatGPT Web 上下文窗口与自动压缩阈值。Enhanced 模式会在安全的工具结果边界建立私有恢复 checkpoint（Pro 约每 100K Token；Plus 根据新页面容量提前建立）。原页面继续运行，仅在失效时使用 checkpoint 与后续正式记录重建。ChatGPT 限制仍然有效，过大的提示会安全失败。更改后请重启 Codex。默认关闭。",
  savedChats: "在 ChatGPT 中保存聊天",
  savedChatsBody: "将 Codex 和 Claude 任务对话保留在 ChatGPT 历史记录中，可能应用 ChatGPT 记忆和自定义指令。API 请求始终使用临时聊天。默认关闭。",
  ...
  manualFreshConversationUnavailable: "仅支持自动化与 Original 会话模式。Enhanced 和零风险模式会强制关闭此设置。",
  ...
  apiAccess: "API 访问（实验性）",
  apiAccessSummary: "面向 Agent 工具的本地 OpenAI 兼容端点，可靠性仍在验证中。",
  apiAccessToggle: "启用 API 访问",
  apiAccessHelpLabel: "了解 API 访问风险",
  apiAccessWarningTitle: "启用 API 访问前请确认",
  apiAccessWarningBody: "此实验性端点不是 OpenAI 官方 API，兼容性尚不完整，运行可能不稳定：回复可能延迟或失败，工具调用与多轮 Agent 流程可能无法在所有客户端可靠运行，请勿用于关键工作。此端点供 Pi 等 Agentic 界面及外部 harness 接入。建议搭配账户安全保护，但本地限制不能保证避免 ChatGPT 限流。API 密钥只应交给可信任的本地工具，切勿公开。每段对话可能耗费较长时间，频繁请求容易触发 429 Too many requests。若出现 429 或账户安全警告，请停止发送请求并检查账户状态。用户须自行承担不当使用造成的账户风险；本项目及开发者不对因此产生的账户限制、暂停使用或封禁负责。",
  apiAccessWarningCancel: "取消",
  apiAccessWarningConfirm: "我已了解，启用",
  ...
  accountSafetyActiveWebTurn: "Web 工作正在执行；完成前无法修改账户安全保护设置。",
