export const zhTW = {
  ...
  apiAccess: "API 存取（實驗性）",
  apiAccessSummary: "供 Agent 工具使用的本機 OpenAI 相容端點，可靠性仍在驗證中。",
  apiAccessToggle: "啟用 API 存取",
  apiAccessHelpLabel: "關於 API 存取風險",
  apiAccessWarningTitle: "啟用 API 存取前請確認",
  apiAccessWarningBody: "這是實驗性端點，並非 OpenAI 官方 API。相容性尚未完整，運作可能不穩定：回覆可能延遲或失敗，工具呼叫與多回合 Agent 流程也可能無法在所有用戶端可靠運作，請勿用於需要可靠結果的關鍵工作。此端點供 Pi 等 Agentic 介面及外部 harness 接入。建議搭配帳戶安全保護，但本機限制無法保證避免 ChatGPT 限流。API Key 只應交給可信任的本機工具，切勿公開。每則對話可能耗費較長時間，頻繁請求容易觸發 429 Too many requests。若出現 429 或帳號安全警告，請停止送出請求並檢查帳號狀態。使用者須自行承擔不當使用造成的帳號風險；本專案及開發者不對因此產生的帳號限制、停權或封鎖負責。",
