...
describe("reversible native Codex route integration", () => {
  test.skipIf(!testFileSymlinks)("route install, update, switching and removal preserve a symlinked shared Codex config", () => {
  ...
  test.skipIf(!testFileSymlinks)("config compensation preserves the link and refuses redirected or invalid targets", () => {
    const { root, codexHome } = fixture();
    const alias = join(codexHome, "config.toml");
  ...
  test("upgrades an active v5 journal and restores its managed feature baseline", () => {
    ...
    const interruptFragment = routeJournal.interruptHook.fragment;
  ...
  test("explicit replacement refuses a custom provider while deactivation restores still-owned route fields", () => {
    const { codexHome } = fixture();
    const configPath = join(codexHome, "config.toml");
    const original = [
      'model = "gpt-5.6-sol"',
