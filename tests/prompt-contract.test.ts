...
function toolRequest(reasoning: "low" | "medium" | "high" | "xhigh" | "max"): CodexParsedRequest {
...
test("tool-capable prompts pass one stable turn token directly to native actions", () => {
  ...
  expect(transportOnly).toContain("sandbox_permissions");
  expect(transportOnly).toContain("justification");
  expect(transportOnly).toContain("prefix_rule");
  expect(transportOnly).toContain("inventory the actual exec_command or shell_command");
  expect(transportOnly).toContain("use codex_tool_call instead of codex_exec");
...
test("Bigger Context compaction preserves history above the retired inline byte budget", () => {
  ...
  expect(stageBytes.reduce((sum, bytes) => sum + bytes, 0)).toBeGreaterThan(CHATGPT_COMPACTION_PROMPT_JSON_BYTE_BUDGET);
