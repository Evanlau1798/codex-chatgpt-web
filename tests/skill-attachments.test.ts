...
const compile = (items: unknown[], enabled = true) => compileChatGptWebPrompt(parse(items), capabilities, undefined, { experimentalSkillAttachments: enabled });
...
test("retained turns reuse prior attachments; fresh chats rebuild them and new skills still upload", () => {
  ...
  expect(compileChatGptWebPrompt(resumed, capabilities, undefined, { experimentalSkillAttachments: true }).skillFiles).toBeUndefined();
  ...
  const compiled = compileChatGptWebPrompt(next, capabilities, undefined, { experimentalSkillAttachments: true });
  expect(compiled.skillFiles).toHaveLength(1);
...
test("skill content counts toward input and final-message budgets, including multipart", () => {
  ...
  for (const parts of [undefined, 3] as const) {
    const compiled = compileChatGptWebPrompt(parsed, capabilities, undefined, { experimentalSkillAttachments: true, experimentalMultipartParts: parts });
...
test("over-limit, malformed and manual requests fail explicitly without silently losing skill content", () => {
  ...
  expect(() => compileChatGptWebPrompt(parse([input(text())]), capabilities, undefined, {
...
test("usage accounts for skills accumulated over many turns without applying a per-message upload cap", () => {
  ...
  const usage = estimateChatGptWebUsage(parsed, { answer: "done" }, capabilities, false, {
