...
test("Claude live smoke selects High for settings and CLI invocations", () => {
  const source = readFileSync(new URL("../scripts/lifecycle-smoke/claude-lane.ts", import.meta.url), "utf8");
  expect(source).toContain('settings.model = "claude-chatgpt-web-high"');
  expect(source).toContain('settings.availableModels.includes("claude-chatgpt-web-high")');
  expect(source).toContain('"--model", "claude-chatgpt-web-high", "--effort", "high"');
  expect(source).not.toContain('"claude-chatgpt-web-pro"');
