...
現在のバージョンは上流 v5.0.6 ベースの `5.0.6-Enhanced.2` です。
...
- **継続的なタスクセッションとネイティブコンパクション。** 連続するメッセージは、タスクに紐付いた
  1 つの一時チャットを 30 分間再利用します。system instructions が同じ場合は差分だけを送り、
  変更された場合は composer へ再送せず、ターンに紐付く context archive から同じチャットへ完全に置き換えます。
  コンテキスト境界に達すると、保持中のエージェントがチェックポイントを書き、
