...
export function compileChatGptWebPrompt(
  ...
  const transportContract = parsed._compactionRequest
    ...
    ? [
      ...
      "For a local text instruction or source file, use codex_tool_inventory with query `__codex_read_file__:<absolute path>` so the outer Codex runtime performs one fixed read-only file operation. Do not use this reserved query for commands.",
