...
1. Install the [latest Enhanced release](https://github.com/Evanlau1798/codex-chatgpt-web/releases/latest). Quit
...
- Keep **Continue in app** for ordinary sign-in. If the account requires a passkey, choose
  **Use Chrome for passkey**; the launcher imports only the verified ChatGPT/OpenAI session into its
  isolated partition and re-verifies the Temporary Chat composer.
- Do not navigate or close the launcher browser while authentication or session verification is
  running.
- Chrome must be installed at the configured path for the passkey option to appear. If Chrome is
  unavailable, the launcher intentionally stays with embedded sign-in rather than pretending that
  an external login was imported.
